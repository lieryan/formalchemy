...
def test_date_8():
    ...
    fs.sync()
    assert_equal(dt.foo, datetime.date(1951, 2, 11))
    assert_equal(dt.bar, datetime.time(4, 6, 8))
    assert_equal(dt.foobar, datetime.datetime(1951, 2, 11, 4, 6, 8))
    session.rollback()
    ...
    assert fs.validate()
    fs.sync()
    assert dt.foo is None
    assert dt.bar is None
    assert dt.foobar is None
    session.rollback()
    ...
    assert_equal(fs.errors[fs.foo], ['Invalid date'])
    ...
    assert not fs.validate()
    assert_equal(fs.errors[fs.bar], ['Invalid time'])
    ...
    assert not fs.validate()
    assert_equal(fs.errors[fs.foobar], ['Incomplete datetime'])
