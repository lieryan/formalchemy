...
from formalchemy.exceptions import PkError
...
def test_fieldset_1():
    fs = FieldSet(order1)
    assert_equal(str(list(fs._raw_fields())), '[AttributeField(id), AttributeField(user_id), AttributeField(quantity), AttributeField(user)]')
    assert_equal(fs.user.name, 'user_id')

def test_fieldset_2():
    fs = FieldSet(bill)
    assert_equal(str(list(fs._raw_fields())), '[AttributeField(id), AttributeField(email), AttributeField(password), AttributeField(name), AttributeField(orders)]')
    assert_equal(fs.orders.name, 'orders')

def test_fieldset_3():
    # binding should not change attribute order:
    fs = FieldSet(User)
    fs_bound = fs.bind(User)
    assert_equal(str(sorted(fs_bound._fields.items())), "[('email', AttributeField(email)), ('id', AttributeField(id)), ('name', AttributeField(name)), ('orders', AttributeField(orders)), ('password', AttributeField(password))]")
...
def test_fieldset_9():
    fs = FieldSet(Two)
    assert_equal(fs.render(), configure_and_render(fs, include=[fs.foo]))
    assert_equal(fs.render(), configure_and_render(fs, exclude=[fs.id]))

def test_fieldset_10():
    fs = FieldSet(Two)
    fs.configure(include=[fs.foo.hidden()])
    assert_equal(fs.render(), '<input id="Two--foo" name="Two--foo" type="hidden" value="133" />')

def test_fieldset_11():
    r"""
    >>> fs = FieldSet(Two)
...
def test_fieldset_14():
    fs = FieldSet(One, data={}, session=session)
    fs.sync()
    session.flush()
    assert_equal(session.query(One).count(), 1)
    session.rollback()

def test_fieldset_15():
    twof = TwoFloat(id=1, foo=32.2)
    fs_twof = FieldSet(twof)
    assert_equal('%.1f' % fs_twof.foo.value, '32.2')
    assert_equal(pretty_html(fs_twof.foo.render()), '<input id="TwoFloat-1-foo" name="TwoFloat-1-foo" type="text" value="32.2" />')

def test_fieldset_16():
    twoi = TwoInterval(id=1, foo=datetime.timedelta(2.2))
    fs_twoi = FieldSet(twoi)
    assert_equal(str(fs_twoi.foo.renderer), '<IntervalFieldRenderer for AttributeField(foo)>')
    assert_equal(fs_twoi.foo.value, datetime.timedelta(2, 17280))
    assert_equal(pretty_html(fs_twoi.foo.render()), '<input id="TwoInterval-1-foo" name="TwoInterval-1-foo" type="text" value="2.17280" />')
    fs_twoi.rebind(data={"TwoInterval-1-foo": "3.1"})
    fs_twoi.sync()
    new_twoi = fs_twoi.model
    assert_equal(new_twoi.foo, datetime.timedelta(3.1))

def test_fieldset_17():
    ...
    assert_equal(pretty_html(fs_twon.foo.render()), '<input id="TwoNumeric-1-foo" name="TwoNumeric-1-foo" type="text" value="2.3" />')
    fs_twon.rebind(data={"TwoNumeric-1-foo": "6.7"})
    fs_twon.sync()
    new_twon = fs_twon.model
    assert_equal(new_twon.foo, Decimal("6.7"))
    
    # test sync when TwoNumeric-1-foo is empty
    fs_twon.rebind(data={"TwoNumeric-1-foo": ""})
    fs_twon.sync()
    new_twon = fs_twon.model
    assert new_twon.foo is None

def test_fieldset_18_a():
    fs_cb = FieldSet(CheckBox)

    assert fs_cb.field.value is None

    # test no checkbox/radio submitted
    fs_cb.rebind(data={})

    assert fs_cb.field.raw_value is None
    assert fs_cb.field.value is False
    assert fs_cb.field.renderer.value is None
    assert_equal(fs_cb.field.render(), '<input id="CheckBox--field" name="CheckBox--field" type="checkbox" value="True" />')
    assert_equal(str(fs_cb.field.renderer), '<CheckBoxFieldRenderer for AttributeField(field)>')
    assert fs_cb.field.renderer._serialized_value() is None
    assert fs_cb.validate()
    assert_equal(fs_cb.errors, {})

    fs_cb.sync()

    cb = fs_cb.model

    assert cb.field is False
...
def test_fieldset_24():
    fs = FieldSet(Two)
    assert_equal(fs.foo.render(), '<input id="Two--foo" name="Two--foo" type="text" value="133" />')

def test_fieldset_25():
    r"""
    >>> fs = FieldSet(Two)
...
def test_fieldset_27():
    # this seems particularly prone to errors; break it out in its own test
    fs = FieldSet(order1)
    assert_equal(fs.user.value, 1)

def test_fieldset_28_a():
    # test re-binding
    fs = FieldSet(Order)
    fs.configure(pk=True, options=[fs.quantity.hidden()])
    fs.rebind(order1)
    assert_equal(fs.quantity.value, 10)
    assert_equal(fs.session, object_session(order1))

def test_fieldset_28_b():
    r"""
    # test re-binding
    >>> fs = FieldSet(Order)
    >>> fs.configure(pk=True, options=[fs.quantity.hidden()])
...
def test_fieldset_30():
    fs = FieldSet(One)
    with assert_raises(ValueError):
        fs.rebind(Two)
    
def test_fieldset_31():
    fs = FieldSet(Two)
    fs.configure()
    fs2 = fs.bind(Two)
    assert_equal([fs2 == field.parent for field in fs2._render_fields.values()], [True])

def test_fieldset_32():
    r"""
...
def test_fieldset_33_a():
    # validation + sync
    fs_2 = FieldSet(Two, session=session, data={'Two--foo': ''})
    assert_equal(fs_2.foo.value, '133') # '' is deserialized to None, so default of 133 is used

    assert fs_2.validate()

    fs_2.configure(options=[fs_2.foo.required()], focus=None)
    assert not fs_2.validate()

    assert_equal(fs_2.errors, {fs_2.foo: ['Please enter a value']})

    fs_2.rebind(data={'Two--foo': 'asdf'})
    assert_equal(fs_2.data, {'Two--foo': 'asdf'})
    assert not fs_2.validate()

    assert_equal(fs_2.errors, {fs_2.foo: ['Value is not an integer']})

    fs_2.rebind(data={'Two--foo': '2'})
    assert_equal(fs_2.data, {'Two--foo': '2'})

    assert fs_2.validate()

    assert_equal(fs_2.errors, {})

    fs_2.sync()

    assert_equal(fs_2.model.foo, 2)

    session.flush()

    with assert_raises_regexp(PkError, r'Primary key of model has changed since binding, probably due to sync\(\)ing a new instance \(from None to 1\)'):
        fs_2.render()
    session.rollback()

def test_fieldset_33_b():
    r"""
    # validation + sync
    >>> fs_2 = FieldSet(Two, session=session, data={'Two--foo': ''})
    >>> fs_2.configure(options=[fs_2.foo.required()], focus=None)
...
def test_fieldset_34():
    fs_1 = FieldSet(One, session=session, data={'One--id': '1'})
    fs_1.configure(pk=True)
    assert fs_1.validate()
    fs_1.sync()
    assert_equal(fs_1.model.id, 1)
    fs_1.rebind(data={'One--id': 'asdf'})
    assert_equal(fs_1.id.renderer.name, 'One--id')
    assert not fs_1.validate()
    assert_equal(fs_1.errors, {fs_1.id: ['Value is not an integer']})
    
def test_fieldset_35():
    # test updating _bound_pk copy
    one = One(id=1)
    fs_11 = FieldSet(one)
    assert_equal(fs_11.id.renderer.name, 'One-1-id')
    one.id = 2
    fs_11.rebind(one)
    assert_equal(fs_11.id.renderer.name, 'One-2-id')

def test_fieldset_36():
    fs_u = FieldSet(User, session=session, data={})
    fs_u.configure(include=[fs_u.orders])
    assert fs_u.validate()
    fs_u.sync()
    assert_equal(fs_u.model.orders, [])
    fs_u.rebind(User, session, data={'User--orders': [str(order1.id), str(order2.id)]})
    assert fs_u.validate()
    fs_u.sync()
    assert_equal(fs_u.model.orders, [order1, order2])
    session.rollback()

def test_fieldset_37_a():
    fs_3 = FieldSet(Three, data={'Three--foo': 'asdf', 'Three--bar': 'fdsa'})
    assert_equal(fs_3.foo.value, 'asdf')
    assert_equal(fs_3.foo.textarea().render(), '<textarea id="Three--foo" name="Three--foo">asdf</textarea>')
    assert_equal(fs_3.foo.textarea("3x4").render(), '<textarea cols="3" id="Three--foo" name="Three--foo" rows="4">asdf</textarea>')
    assert_equal(fs_3.foo.textarea((3,4)).render(), '<textarea cols="3" id="Three--foo" name="Three--foo" rows="4">asdf</textarea>')
    assert_equal(fs_3.bar.value, 'fdsa')
...
def test_fieldset_46():
    # test setter rejection
    fs = FieldSet(One)
    with assert_raises_regexp(AttributeError, r'Do not set field attributes manually.  Use append\(\) or configure\(\) instead'):
        fs.id = fs.id.required()

def test_fieldset_47():
    r"""
    # join
...
def test_fieldset_48():
    ...
    assert_equal(v.id, 1)
    session.refresh(v)
    assert_equal(v.start.x, 10)
    assert_equal(v.end.y, 40)
    session.rollback()

def test_fieldset_49():
    r"""
    # readonly tests
    >>> t = FieldSet(john)
    >>> john.name = None
...
def test_fieldset_50():
    fs_or = FieldSet(order1)
    assert_equal(fs_or.user.render_readonly(), '<a href="mailto:bill@example.com">Bill</a>')

def test_fieldset_51_a():
    out = FieldSet(OrderUserTag, session=session)
    assert_equal(
        list(str(x) for x in sorted(out._fields)),
        ['id', 'order_id', 'order_user', 'tag', 'user_id'],
    )
    assert_equal(out.order_user.name, 'order_user')
    assert out.order_user.is_raw_foreign_key is False
    assert out.order_user.is_composite_foreign_key is True
    assert_equal(
    ...
    assert out.validate()
    out.sync()
    assert_equal(str(out.model.order_user), "OrderUser(1, 2)")

def test_fieldset_51_b():
    r"""
    >>> out = FieldSet(OrderUserTag, session=session)
...
def test_fieldset_52():
    fs = FieldSet(Function)
    fs.configure(pk=True)
    assert fs.foo.render().startswith('<span')

def test_fieldset_53():
    fs_r = FieldSet(Recursive)
    assert fs_r.parent_id.is_raw_foreign_key is True
    fs_r.rebind(data={'Recursive--foo': 'asdf'})
    assert fs_r.validate()

def test_fieldset_54():
    r"""
...
def test_fieldset_55():
    fs_oo = FieldSet(OptionalOrder)
    fs_oo.rebind(data={'OptionalOrder--user_id': fs_oo.user_id._null_option[1], 'OptionalOrder--quantity': ''})
    assert fs_oo.validate()
    assert fs_oo.user_id.value is None

def test_fieldset_56():
    fs_bad = FieldSet(One)
    with assert_raises_regexp(ValueError, r'Unrecognized Field `Field\(invalid\)` in `include` -- did you mean to call append\(\) first\?'):
        fs_bad.configure(include=[Field('invalid')])

@unittest.skip('expected failure')
def test_fieldset_57():
    fs_s = FieldSet(Synonym)
    assert_equal(
        repr(sorted(fs_s._fields.items())),
        "[('_foo', AttributeField(_foo)), ('id', AttributeField(id))]",
    )

@unittest.skip('expected failure')
def test_fieldset_58():
...
def test_fieldset_62():
    fs_prop = FieldSet(Property)
    assert fs_prop.foo.is_readonly() is True

def test_fieldset_63():
    fs_conflict = FieldSet(ConflictNames)
    fs_conflict.rebind(conflict_names)
    assert fs_conflict.render().startswith('<div>')

@unittest.skip('expected failure')
def test_fieldset_64():
    fs_syn = FieldSet(Synonym)
    assert_equal(fs_syn.render(),
"""\
<div>
...
@unittest.skip('expected failure')
def test_fieldset_65():
    fs_rec = FieldSet(RecursiveChild, session=session)
    assert_equal(fs_rec.render(),
